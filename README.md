...
TypeScript SDK for the Nouxis Solana protocol — register AI agents as NFTs, manage reputation, delegate authority, and settle NouX payments on-chain.
...
import { IDL, Nouxis } from "@Nouxis-ai/sdk/idl";
import { NETWORKS, registerAgent, hashTag, hashBytes } from "@Nouxis-ai/sdk";

const connection = new Connection(clusterApiUrl("devnet"));
const provider = new AnchorProvider(connection, wallet, {});
const config = NETWORKS.devnet;
const program = new Program<Nouxis>(IDL, config.programId, provider);
```
...
const { instruction, servicePda } = await addService(program, {
  ...
  endpoint: "https://agent.Nouxis.ai/.well-known/agent-card.json",
  version: "1.0.0",
});
...
await updateService(program, {
  ...
  nftMint: agentMint,
  serviceType: ServiceType.A2A,
  newEndpoint: "https://v2.agent.Nouxis.ai/agent-card.json",
});
...
```ts
import { giveFeedback, revokeFeedback, hashTag, zeroBytes32 } from "@Nouxis-ai/sdk";
...
```ts
import { validationRequest, validationResponse, hashBytes } from "@Nouxis-ai/sdk";
...
```ts
import { DelegationBuilder, createDelegation, revokeDelegation } from "@Nouxis-ai/sdk";
...
```ts
import { parseNouxisError, NouxisErrorCode } from "@Nouxis-ai/sdk";

try {
  await sendTransaction(tx);
} catch (err) {
  const parsed = parseNouxisError(err);
  if (parsed?.code === NouxisErrorCode.AgentInactive) {
...
| Path | Description |
|------|-------------|
| `@Nouxis-ai/sdk` | Core SDK (identity, service, reputation, validation, delegation, payment) |
| `@Nouxis-ai/sdk/settlement` | Settlement instruction builders (`buildVerifyAndSettleIx`, `buildCloseReceiptIx`) |
| `@Nouxis-ai/sdk/idl` | Anchor IDL (JSON + TypeScript types) |
